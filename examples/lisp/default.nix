...
in
  ...
  with builtins; let
    ...
    identifier = let
      validChar = c: match ''[a-zA-Z0-9+|:@_-]'' c != null;
    in
      listLexeme (takeWhile1 validChar);
    ...
    racketInfoExtractor = parsedLisp: let
    ...
    racketPkgsExtractor = parsedLisp: let
      name = head parsedLisp;
      value = elemAt (tail (tail parsedLisp)) 0;
      value' =
        listToAttrs
        (map
          (list: lib.attrsets.nameValuePair (head list) (cleanValues (tail list)))
          value);
    in
      lib.attrsets.nameValuePair name value';
